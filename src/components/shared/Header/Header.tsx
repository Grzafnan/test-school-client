                        ...
                        <nav aria-label="Global">
                            <ul className="flex items-center gap-6 text-sm">
                                {menus.map((menu, index) => (
                                    <li key={index}>
                                        <Link className="text-gray-500 transition hover:text-gray-500/75 font-medium" to={menu.href}>
                                            {menu.name}
                                        </Link>
                                    </li>
                                ))}
            ...
            >
                <div className="flex flex-col h-full">
                    ...
                    <nav className="flex-1 px-4 py-6">
                        <ul className="space-y-4">
                            {menus.map((menu, index) => (
                                <li key={index}>
                                    <Link
                                        className="block py-3 px-4 text-lg font-medium text-gray-700 hover:text-teal-600 hover:bg-gray-50 rounded-lg transition-all duration-200"
                                        to={menu.href}
                                        onClick={closeMobileMenu}
                                    >
                                        {menu.name}
                    ...
                    <div className="p-4 border-t border-gray-200 space-y-3">
                        <Link
                            className="block w-full text-center rounded-md bg-teal-600 px-5 py-3 text-sm font-medium text-white shadow-sm hover:bg-teal-700 transition-colors"
                            to="/login"
                            onClick={closeMobileMenu}
                        >
                            Login
                        ...
                        <Link
                            className="block w-full text-center rounded-md bg-gray-100 px-5 py-3 text-sm font-medium text-teal-600 hover:bg-gray-200 transition-colors"
                            to="/register"
                            onClick={closeMobileMenu}
                        >
                            Register
